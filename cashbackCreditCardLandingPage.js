var MM09Help = {
    init: function () {
        if (window.location.pathname.indexOf('/credit-card/cashback-credit-card') !== -1) {
            // MM09Help.HeroBanner();
            MM09Help.addBodyClass();
            setTimeout(function () {
                MM09Help.rewardsCompare();
                MM09Help.HeroBanner();
                MM09Help.feesConditionsCta();
                // MM09Help.featuredBlogPosts();
                // MM09Help.addDigitalCardSection();
                MM09Help.addFeatureBlogAndDigitalCardSections();
                MM09Help.sectionCtabox_1();
                MM09Help.addReviewsCta();
                MM09Help.moveDisclaimerSibling();
                MM09Help.addPerksUseClass();
            }, 300);
        } else {
            //document.querySelector('body').classList.remove('crd-apply-btn-cta');
            console.log("document.querySelector('header.wrap-header + div + div'): ", document.querySelector('header.wrap-header + div + div.relative'));
            document.querySelector('.remap-cashback-landing')?.classList.remove('remap-cashback-landing');

            // setTimeout(function () {
            //     document.querySelector('header.wrap-header + div + div.relative')?.classList.add('custom-banner');
            //     document.querySelector('.cashback-hero')?.remove();
            //     document.querySelector('.finder-verified')?.remove();
            //     document.querySelector('.award-winning-section')?.remove();
            //     if (document.querySelector('.award-winning-section-bottom')) {
            //         document.querySelector('.award-winning-section-bottom').classList.remove('award-winning-section-bottom');
            //     }
            //     document.querySelector('.cashback-features')?.remove();
            //     document.querySelector('.rewards-compare')?.remove();
            //     document.querySelectorAll('.cashback-cta-wrap').forEach(function (element) {
            //         element.remove();
            //     })
            //     document.querySelector('.digital-card')?.remove();
            //     document.querySelector('.featured-posts')?.remove();
            //     //document.querySelector('#spyReviews')?.classList.remove('remap-spy-reviews');
            //     //document.querySelector('#perks-use')?.classList.remove('remap-perks-use');
            //     document.querySelector('.faq-section')?.classList.remove('faq-section');
            // }, 300);
        }
    },
    HeroBanner: function () {
        document.querySelector('.cashback-hero')?.remove();
        document.querySelector('.finder-verified')?.remove();
        var heroBanner = document.querySelector('header.wrap-header + div + div.relative');
        heroBanner.classList.remove('custom-banner');
        if (heroBanner && !document.querySelector('.cashback-hero')) {
            ...
            MM09Help.moveAwardSection();
        } else {
            setTimeout(MM09Help.HeroBanner, 30);
        }
    },
    // Add the body class
    addBodyClass: function () {
        ...
        if (targetBody) {
            targetBody.classList.add('remap-cashback-landing');
        } else {
            setTimeout(MM09Help.addBodyClass, 30);
        }
    },
    // Move the div immediately after #scrollDisclaimer (the "#scrollDisclaimer + div"
    moveDisclaimerSibling: function () {
        ...
        if (disclaimer && nextDiv && nextDiv.tagName === 'DIV') {
            ...
            setTimeout(MM09Help.moveDisclaimerSibling, 30);
        }
    },
    // Add the .faq-section class to the div immediately after .featured-posts
    addFaqSectionClass: function () {
        var el = document.querySelector('.featured-posts + div');
        if (el) {
            el.classList.add('faq-section');
        } else {
            setTimeout(MM09Help.addFaqSectionClass, 30);
        }
    },
    addPerksUseClass: function () {
        var el = document.querySelector('#perks-use');
        if (el) {
            el.classList.add('remap-perks-use');
        } else {
            setTimeout(MM09Help.addFaqSectionClass, 30);
        }
    },

    // Add the .earn-rewards-count class + shared CTA to the Perks-use CTA-box section
    sectionCtabox_1: function () {
        var el = document.querySelector('#perks-use + div');
        if (!el) {
            setTimeout(MM09Help.sectionCtabox_1, 30);
            return;
        }
        if (el.classList.contains('earn-rewards-count')) return;
        ...
        if (targetP) {
            targetP.insertAdjacentHTML('afterend', MM09Help.ctaWrap('earn-rewards-count__cta-wrap'));
        }
    },
    // Add the .earn-rewards-count class + shared CTA to the FAQ CTA-box section
    sectionCtabox_2: function () {
        var el = document.querySelector('.faq-section + div');
        if (!el) {
            setTimeout(MM09Help.sectionCtabox_2, 30);
            return;
        }
        if (el.classList.contains('earn-rewards-count')) return;
    ...
    addFeatureBlogAndDigitalCardSections: function () {
        var reviewEl = document.querySelector('#spyReviews');
        if (reviewEl) {
            MM09Help.featuredBlogPosts(reviewEl);
            MM09Help.addDigitalCardSection(reviewEl);

            reviewEl.classList.add('remap-spy-reviews')
        } else {
            setTimeout(MM09Help.featuredBlogPosts, 30);
        }
    },
    // Insert the "A clearer guide to credit cards" 3-card section after #spyReviews.
    featuredBlogPosts: function (reviewEl) {
        if (reviewEl) {
            ...
            var cards = [
                {
                    ...
                    link: '/blog/cashback-reward-credit-card',
                ...
                },
                {
                    ...
                    link: '/blog/how-to-save-on-your-energy-bills',
                ...
                },
                {
                    ...
                    link: '/blog/credit-card-loan-buy-now-pay-later-whats-best-for-me',
                    tags: [['finance', 'Personal Finance'], ['health', 'Financial Health'], ['featured', 'Featured']],
            ...
            reviewEl.insertAdjacentHTML('afterend', `
            ...
            MM09Help.addFaqSectionClass();
            MM09Help.sectionCtabox_2();
        } else {
            setTimeout(MM09Help.featuredBlogPosts, 30);
        }
    },
    // Insert the shared CTA block immediately after the #spyReviews
    addReviewsCta: function () {
        ...
        if (carousel) {
            ...
            carousel.insertAdjacentHTML('afterend', MM09Help.ctaWrap('reviews__cta-wrap'));
        } else {
            setTimeout(MM09Help.addReviewsCta, 30);
        }
    },
    // Insert the "digital card" section (text + image + shared CTA) right after
    addDigitalCardSection: function (reviewEl) {
         document.querySelector('.digital-card')?.remove();
        if (reviewEl) {
            if (!document.querySelector('.digital-card')) {
                reviewEl.insertAdjacentHTML('beforebegin', `
                        ...
                        <div class="container-narrowed">
                            <div class="digital-card__grid grid grid-cols-1 lg:grid-cols-2 items-center">
                                <div class="digital-card__content">
                                    <h2 class="digital-card__title font-[sharpGFamily]">
                                        A digital card, ready when you are
                                    </h2>

                                    <p class="digital-card__text">
                                        Approvals in as little as 60 minutes, and you're ready to go.
                                        No card. No waiting. Just add your digital card to your mobile
                                        wallet and start spending from the moment you're approved.
                                        <sup>4</sup>
                                    </p>
                                </div>
                            ...
                            ${MM09Help.ctaWrap('digital-card__cta-wrap')}
                        </div>
                    </div>
                </section>
            `);
    ...
    // Insert the "Rewards you can spend" comparison section after the award section.
    rewardsCompare: function () {
        
        document.querySelector('.rewards-compare')?.remove();

        var target = document.querySelector('.award-winning-section');
        ...
        if (target) {
            ...
            MM09Help.addCashbackFeatures();
        } else {
            setTimeout(MM09Help.rewardsCompare, 30);
        }
    },
    // the .rewards-compare section. Polls until .rewards-compare exists.
    addCashbackFeatures: function () {
        document.querySelector('.cashback-features')?.remove();
        var rewardsCompare = document.querySelector('.rewards-compare');
        if (!rewardsCompare) {
            setTimeout(MM09Help.addCashbackFeatures, 30);
            return;
        }
        if (document.querySelector('.cashback-features')) return;
        ...
        rewardsCompare.insertAdjacentHTML('beforebegin', `
            <section class="cashback-features mt-[80px] mb-[80px]">
                <div class="container mx-auto">
                    ...
                    ${MM09Help.ctaWrap('cashback-features__cta-wrap')}
                </div>
            </section>
        `);
    },
    // Append the shared CTA block to the bottom of the section#spyRatesAndFees
    feesConditionsCta: function () {
        var container = document.querySelector('section#spyRatesAndFees .container');
        if (container) {
            if (!document.querySelector('.cashback-cta-wrap.fees-conditions__cta-wrap')) {
                container.insertAdjacentHTML('afterend', MM09Help.ctaWrap('fees-conditions__cta-wrap'));
            }
        } else {
            setTimeout(MM09Help.feesConditionsCta, 30);
        }
    },
    // Find the existing "Award-winning lender" section on the page
    moveAwardSection: function () {
        var awardSection = document.querySelector('section.container + div');
        if (awardSection) {
            var clone = awardSection.cloneNode(true);
            clone.classList.add('award-winning-section');
            clone.style.opacity = '1';
            awardSection.classList.add('award-winning-section-bottom');
            var finderSection = document.querySelector('.finder-verified');
            finderSection.insertAdjacentElement('afterend', clone);
        } else {
            setTimeout(MM09Help.moveAwardSection, 30);
        }
    },
};

(function pollForMM09() {
    if (window.MM09) return false;
    if (document.querySelector('#__nuxt')) {
        window.MM09 = { experiment: 'MM09', variation: 'Variation 1', audience: 'All Devices', version: '1.0.01' };
        console.log(MM09);
        MM09Help.init();
